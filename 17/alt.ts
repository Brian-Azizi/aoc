...
function processNode(point, deltaRow, deltaCol, futurePoints, walkingRound) {
  ...
  if (direction == NORTH) {
    handleFromNorth(node, previous, futurePoints, walkingRound);
    return;
  }

  if (direction == SOUTH) {
    handleFromSouth(node, previous, futurePoints, walkingRound);
    return;
  }

  if (direction == WEST) {
    handleFromWest(node, previous, futurePoints, walkingRound);
    return;
  }

  if (direction == EAST) {
    handleFromEast(node, previous, futurePoints, walkingRound);
    return;
  }
}
...
function handleFromNorth(node, previous, futurePoints, walkingRound) {
  ...
  if (hl + best < node.fromNorth1) {
    node.fromNorth1 = hl + best;
    changed = true;
  }

  if (hl + previous.fromNorth1 < node.fromNorth2) {
    node.fromNorth2 = hl + previous.fromNorth1;
    changed = true;
  }

  if (hl + previous.fromNorth2 < node.fromNorth3) {
    node.fromNorth3 = hl + previous.fromNorth2;
    changed = true;
  }
...
function handleFromSouth(node, previous, futurePoints, walkingRound) {
  ...
  if (hl + best < node.fromSouth1) {
    node.fromSouth1 = hl + best;
    changed = true;
  }

  if (hl + previous.fromSouth1 < node.fromSouth2) {
    node.fromSouth2 = hl + previous.fromSouth1;
    changed = true;
  }

  if (hl + previous.fromSouth2 < node.fromSouth3) {
    node.fromSouth3 = hl + previous.fromSouth2;
    changed = true;
  }
...
function handleFromWest(node, previous, futurePoints, walkingRound) {
  ...
  if (hl + best < node.fromWest1) {
    node.fromWest1 = hl + best;
    changed = true;
  }

  if (hl + previous.fromWest1 < node.fromWest2) {
    node.fromWest2 = hl + previous.fromWest1;
    changed = true;
  }

  if (hl + previous.fromWest2 < node.fromWest3) {
    node.fromWest3 = hl + previous.fromWest2;
    changed = true;
  }
...
function handleFromEast(node, previous, futurePoints, walkingRound) {
  ...
  if (hl + best < node.fromEast1) {
    node.fromEast1 = hl + best;
    changed = true;
  }

  if (hl + previous.fromEast1 < node.fromEast2) {
    node.fromEast2 = hl + previous.fromEast1;
    changed = true;
  }

  if (hl + previous.fromEast2 < node.fromEast3) {
    node.fromEast3 = hl + previous.fromEast2;
    changed = true;
  }
...
function pushToFuturePoints(futurePoints, walkingRound, node, changed) {
  ...
  if (node.selectedRound == walkingRound) {
    return;
  }
